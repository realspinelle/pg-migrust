...
const binp = path.join(cargoDir, "bin", "pg-migrust");

if (fs.existsSync(binp)) {
  console.log("Uninstalling pg-migrust...");
  exec(`cargo uninstall pg-migrust`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("pg-migrust not found skipping!");
}
